...
sim_lm_data_NI_case <- function(N = 100, K = 4, seed = 1234) {
  set.seed(seed)
  ####################SIMULATION OF DATA
...
linearCodeNI_better <- nimbleCode({
...
cases <- list()
cases[["N=100, p=4"]]   <- sim_lm_data_NI_case(N=100,  K=4,   seed = 431234)
cases[["N=1000, p=4"]]  <- sim_lm_data_NI_case(N=1000, K=4,   seed = 431234)
cases[["N=100, p=16"]]  <- sim_lm_data_NI_case(N=100,  K=16,  seed = 431234)
cases[["N=1000, p=16"]] <- sim_lm_data_NI_case(N=1000, K=16,  seed = 431234)
cases[["N=30, p=50"]]   <- sim_lm_data_NI_case(N=30,   K=50,  seed = 431234)
...
for(case_name in names(cases)) {
  results_NI_BFG[[ case_name ]] <- run_nimble(linearCodeNI, cases[[case_name]])
}
save(results_NI_BFG, file = "results_NI_BFG.RData")

# Block 2:
results_NI_better <- list()
for(case_name in names(cases)) {
  results_NI_better[[ case_name ]] <- run_nimble(linearCodeNI_better, cases[[case_name]])
}
save(results_NI_better, file = "results_NI_better.RData")
