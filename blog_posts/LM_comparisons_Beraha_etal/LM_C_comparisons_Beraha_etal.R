...
sim_lm_data_conj_case <- function(N = 100, K = 4, seed = 1234) {
  ...
  ##### We assume the second one is what was used, because
  ##### it over-writes values from the first.
#  for(i in 2:K){
...
cases <- list()
cases[["N=100, p=4"]]   <- sim_lm_data_conj_case(N=100,  K=4,  seed = 1) # 1 is in BFG's code
...
cases[["N=2000, p=30"]] <- sim_lm_data_conj_case(N=2000, K=30, seed = 1)
...
# Simpler, better code, with "var=sigmasq" to detect conjugacy for sigmsq
# This code is not used for results in the blog post
linearCodeZconj2 <- nimbleCode({ #Has detected conjugacy for sigmasq
  # This doesn't make much difference in efficiency.
  mu[1:n] <- (X[1:n,1:k] %*% beta[1:k])[1:n,1] # alternate
  for(i in 1:n) {
    # mu[i] <- inprod(X[i,], beta[1:k])
    # likelihood
...
for(case_name in names(cases)) {
  results_C_BFG[[ case_name ]] <- run_nimble(linearCodeZ, cases[[case_name]])
}
save(results_C_BFG, file = "results_C_BFG.RData")

# Block 2:
# Here are results from making the model code simpler and better.
# This actually achieves conjugacy for the beta[i]s
results_C_better <- list()
for(case_name in names(cases)) {
  results_C_better[[ case_name ]] <- run_nimble(linearCodeZconj, cases[[case_name]])
}
save(results_C_better, file = "results_C_better.RData")

# Block 3 (not repored in blog post)
# Here are results when conjugacy for sigmasq is also detected
## results_C_better2 <- list()
## for(case_name in names(cases)) {
##   results_C_better2[[ case_name ]] <- run_nimble(linearCodeZconj2, cases[[case_name]])
## }
## save(results_C_better2, file = "results_C_better2.RData")
