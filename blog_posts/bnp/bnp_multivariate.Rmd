...
load(url("http://r-nimble.org/nimbleExamples/seizures.Rda"))
names(seizures)  
head(seizures)
```
...
pairMatrix <- matrix(0, nrow = n, ncol = n)
for(iter in 1:niter) { 
  for (i in 1:(n-1)) { 
    for(j in (i+1):n) {
      if(xiSamples[iter, i] == xiSamples[iter, j]) {
        pairMatrix[i, j] = pairMatrix[i, j] + 1
      }
    }
  }
}
pairMatrix <- pairMatrix + t(pairMatrix)  # to complete lower diagonal
pairMatrix <- pairMatrix / niter + diag(1, n) # to complete diagonal
...
reorder.pairMatrix  = pairMatrix[newOrder, newOrder]
image.plot(1:n, 1:n, reorder.pairMatrix , col = collist(6),
