...
Checking the assigned samplers, we can see that indicator parameters are assigned to an `RJ_indicator` sampler whose `targetNode` is the corresponding coefficient, while `beta` parameters are assigned to a `toggled` sampler, which is used only when its target parameter is included in the model.
...
set.seed(100)
system.time(samples_nimble <- runMCMC(CMCMCRJ, niter = 100000, nburnin = 10000, thin = 10))
```
...
zCols <- which(colnames(samples_nimble) %in% zNames)
