...
In earlier versions of NIMBLE, to allow a user-defined distribution, we required one provide an "r" function for random number generation. That's not required any longer unless any algorithms you use would need to generate from the distribution (e.g., generating from a prior or from the data distribution), but we show it here anyway.
