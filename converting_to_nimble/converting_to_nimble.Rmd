...
3.  Choose how you want to run MCMC.
  ...
  set up and run an MCMC using   NIMBLE's default configuration.
   ...
   and build the MCMC, and compile both the model and MCMC.  Then run the MCMC  using `runMCMC` or by calling the `run` function of compiled MCMC.  See the NIMBLE User Manual to learn more about what you can do.

## Smaller steps you may need for converting existing code

If the main steps above aren't sufficient, consider these additional steps.
...
5. Possibly split the `data` into `data` and
  ...
  model, such as `nsite` in `for(i in 1:nsite) {...}` and constant vectors of factor indices.
    - Data are
...
6.  Possibly update initial values (`inits`).
    ...
    - When you build a model with `nimbleModel` for more control than `nimbleMCMC`, you can provide `inits` as a list.  This sets defaults than can be over-ridden with the `inits` argument to `runMCMC`.
...
This is known as an "N-mixture" model in ecology.  The details aren't really important for illustrating the mechanics of converting this model to NIMBLE, but here is a brief summary anyway.  The latent abundances `N[i]` at sites `i = 1...M` are assumed to follow a Poisson.  The  j-th count at the i-th site, `C[i, j]`, is assumed to follow a binomial.  The abundance at each site depends on a habitat-specific intercept and coefficient for vegetation height, with a log link.  The detection probability for each sampling occasion is assumed to depend on a date-specific intercept and coefficient for wind speed.  K&#233;ry and Royle concocted this as a simulated example to illustrate the hierarchical modeling approaches for estimating abundance from count data on repeated visits to multiple sites.
