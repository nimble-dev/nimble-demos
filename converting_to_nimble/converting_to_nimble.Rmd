...
You can use  `model$initializeInfo()` to see which nodes are not initialized. You can also call `model$calculate()` to see if the model has a valid log probability density or `model$calculate(nodes)` to see if the log probability density for the set of `nodes` is valid.