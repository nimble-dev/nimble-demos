...
library(coda) ## for computing ESS
```

***(CHRIS:  There are a number of places were it would seem approprite to add references to well known papers.  Is this ok with you?  I am not sure how to automatically add references in Markdown, maybe we just type them by hand?  Just checking in case there is something simpler.)***
...
cMCMC$run(1100)                                                                    # running the MCMC computation
```
...
itersave <- 101:1100                                                               # samples to use for posterior # plots: 
ts.plot(samples[itersave, "alpha"], xlab="iteration", ylab=expression(alpha)) # Trace plot of the posterior samples of the concentration parameter
hist(samples[itersave, "alpha"], xlab=expression(alpha), main="", ylab="Frequency") # Histogram of the posterior samples for the concentration parameter 
quantile(samples[itersave, "alpha"], c(0.5, 0.025, 0.975))
```
...
alphaSamples <- samples[itersave, 1] # posterior samples of the concentration parameter
muTildeSamples <- samples[itersave, 2:273] # posterior samples of the mean cluster parameter
s2TildeSamples <- samples[itersave, 274:545] # posterior samples of the variance cluster parameter
xiSamples <- samples [itersave, 546:817] # posterior samples of the membership cluster parameter

grid <- seq(-2.5, 2.5, len = 200) # grid for new observation y.tilde

postPredictiveSamples <- matrix(0, ncol = length(grid), nrow = length(itersave))
for(i in 1:length(itersave)){
...
# plots:
hist(data$y, freq=FALSE, xlim=c(-2.5, 2.5), ylim=c(0,0.7), main = "", xlab = "Log waiting times") # histogram of standardized log waiting time
...
truncG <- 29 # truncation level for G, obtained from running the getSamplesDPmeasure(cMCMC) line

probY70 <- c() # posterior samples of P(y.tilde > 70)
for(i in 1:length(itersave)){
  probY70[i] <- sum(samplesG[i, 1:truncG] * pnorm(-0.06281407, 
                              samplesG[i, (2*truncG+1):(3*truncG)], 
                              sqrt(samplesG[i, (truncG+1):(2*truncG)]), 
                              lower.tail = FALSE)) 
}
...
```{r, results-CRPalpha2}
samples <- as.matrix(cMCMC$mvSamples) # posterior samples
itersave <- 101:1100                                                               # samples to use for posterior computation
# plots: 
ts.plot(samples[itersave, 1], xlab="iteration", ylab=expression(alpha)) # Trace plot of the posterior samples of the concentration parameter
hist(samples[itersave, 1], xlab=expression(alpha), ylab="Frequency", main="") # Histogram of the posterior samples for the concentration parameter 
```
...
betaStarSamples <- samples[itersave, 2:51]
lambdaStarSamples <- samples[itersave, 52:101]
wSamples <- samples[itersave, 102:151]

grid <- seq(40, 100, len=200)
Trunc <- consts$Trunc

postPredicitveSamples <- matrix(0, ncol = length(grid), nrow = length(itersave))
for(i in 1:length(itersave)) {
...
***(CHRIS:  I wonder if the path for the file should be online somewhere, so that people can run the code if they want and see it work ...)***
...
One useful way to think about this specifiation is as an extension to the mixture of normal distributions we discussed earlier in this blog post.
