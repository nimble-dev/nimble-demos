...
Recently, we added support for Markov chain Monte Carlo (MCMC) inference of Bayesian nonparametric (BNP) mixture models to NIMBLE. In particular, starting with version 0.6-11, NIMBLE provides functionality for fitting models involving Dirichlet process priors using either the Chinese Restaurant Process (CRP), or a truncated stick-breaking (SB) representation of the Dirichlet process prior.
...
conf <- configureMCMC(model, monitors = c("xi", "muTilde", "s2Tilde", "alpha")) 
mcmc <- buildMCMC(conf)          
cmcmc <- compileNimble(mcmc, project = model)                                   
samples <- runMCMC(cmcmc, niter = 12000, nburnin = 2000, setSeed = TRUE) # running the MCMC computation
```
...
# plots: 
ts.plot(samples[ , "alpha"], xlab = "iteration", ylab = expression(alpha)) # Trace plot for the concentration parameter
hist(samples[ , "alpha"], xlab = expression(alpha), main = "", ylab = "Frequency") # Posterior histogram
...
muTildeSamples <- samples[ , grep('muTilde', colnames(samples))] # posterior samples of the cluster means
s2TildeSamples <- samples[ , grep('s2Tilde', colnames(samples))] # posterior samples of the cluster variances
xiSamples <- samples [ , grep('xi', colnames(samples))] # posterior samples of the cluster memberships

standlgrid <- seq(-2.5, 2.5, len = 200) # standardized grid in log scale

densitySamplesStandl <- matrix(0, ncol = length(standlgrid), nrow = nrow(samples)) # density samples for standardized grid in log scale
...
# plots:
hist(data$y, freq = FALSE, xlim = c(-2.5, 2.5), ylim = c(0,0.75), main = "", xlab = "Waiting times in standardized log scale")
lines(standlgrid, apply(densitySamplesStandl, 2, mean), lwd = 2, col = 'black') # pointwise estimate of the density for standardized log grid
...
lgrid <- standlgrid*sd(lFaithful) + mean(lFaithful) # grid in log scale
densitySamplesl <- densitySamplesStandl / sd(lFaithful) # density samples for grid in log scale

hist(faithful$waiting, freq = FALSE, xlim = c(40, 100), ylim=c(0, 0.05), main = "", xlab = "Waiting times") 
lines(exp(lgrid), apply(densitySamplesl, 2, mean)/exp(lgrid), lwd = 2, col = 'black') # pointwise estimate of the density for original data
...
The following code generates posterior samples from the random measure $G$. The 'cmcmc' object includes the model and posterior samples from the parameters. The  getSamplesDPmeasure() function returns a list  with 'trunc' and 'samples'. 'trunc' returns the trucation level of the random measure $G$ and 'samples' returns samples from $G$ in a matrix with $(truncG * (p+1))$ columns, where $p$ is the dimension of the vector of parameters with distribution $G$ (in this example $p=2$).
```{r, G-normaldensity}
outputG <- getSamplesDPmeasure(cmcmc) 
samplesG <- outputG$samples
```

The following code computes posterior samples of $P(\tilde{y} > 70)$ using the posterior samples from the random measure $G$. Note that these samples are computed based on the transformed model and a value larger than 70 corresponds to a value larger than 0.03557236 in the previously defined grid.
...
model <- nimbleModel(code, data = data, inits = inits, constants = consts)
cmodel <- compileNimble(model)
conf <- configureMCMC(model, monitors = c("xi", "betaTilde", "lambdaTilde", "alpha"))
mcmc <- buildMCMC(conf)
cmcmc <- compileNimble(mcmc, project = model)
samples <- runMCMC(cmcmc, niter = 12000, nburnin = 2000, setSeed = TRUE)
```
...
```{r, results-CRPalpha2}
ts.plot(samples[ , 'alpha'], xlab = "iteration", ylab = expression(alpha)) # Trace plot of concentration parameter
hist(samples[ , 'alpha'], xlab = expression(alpha), ylab = "Frequency", main = "") # Histogram of concentration parameter
```
...
truncG <- outputG$trunc
grid <- seq(40, 100, len = 200)

weightSam <- samplesG[ , grep('weight', colnames(samplesG))]
betaTildeSam <- samplesG[ , grep('betaTilde', colnames(samplesG))] 
lambdaTildeSam <- samplesG[ , grep('lambdaTilde', colnames(samplesG))]
...
for(iter in seq_len(nrow(samples))) {
  densitySamples[iter, ] <- sapply(grid, function(x) 
    sum( weightSam[iter, ] * dgamma(x, shape = betaTildeSam[iter, ], 
                scale = lambdaTildeSam[iter, ]))) 
}


hist(faithful$waiting, freq = FALSE, xlim = c(40,100), ylim = c(0, .045), main = "", ylab = "", xlab = "Old Faithful data")
lines(grid, apply(densitySamples, 2, mean), lwd = 2, col = 'black')
lines(grid, apply(densitySamples, 2, quantile, 0.1), lwd = 2, lty = 2, col = 'black')
lines(grid, apply(densitySamples, 2, quantile, 0.9), lwd = 2, lty = 2, col = 'black')
```
...
An alternative representation of the Dirichlet process mixture uses the stick breaking representation of the random distribution $G$, [@sethuraman_94].  NIMBLE allows us to specify an approximation that involves a truncation of the Dirichlet process to a finite number of atoms of $L$.  The resulting model therefore reduces to a finite mixture with $L$ components and a very particular prior on the weights of the mixture components.
...
with $v_l \mid \alpha \sim Beta(1, \alpha), l=1, \ldots, L-1$.  The following code provides the NIMBLE specification for the model:
...
betaStarSam <- samples[ , grep('betaStar', colnames(samples))]
lambdaStarSam <- samples[ , grep('lambdaStar', colnames(samples))]
weightSam <- samples[ , grep('w', colnames(samples))]
...
for(i in 1:nrow(samples)) {
  densitySamples[i, ] <- sapply(grid, function(x) 
    sum(weightSam[i, ] * dgamma(x, shape = betaStarSam[i, ],
                                          scale = lambdaStarSam[i, ])))
}

hist(faithful$waiting, freq = FALSE,  main = "Old Faithful data", ylim=c(0,0.05)) # histogram of original data
lines(grid, apply(densitySamples, 2, mean), lwd = 2, col = 'black') 
