...
The following code generates posterior samples from the random measure $G$. The 'cMCMC' object includes the model and posterior samples from the parameters. When running the getSamplesDPmeasure() function, a message displays the truncation level of $G$, say $truncG$. The resulting object is a matrix having $(truncG * (p+1))$ colums, where $p$ is the dimension of the vector of parameters with distribution $G$ (in this example $p=2$).
```{r, G-normaldensity}
