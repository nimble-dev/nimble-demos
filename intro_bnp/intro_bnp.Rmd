...
We first consider a location-scale Dirichlet process mixture of normal distributions fitted to the transformed data $y_i^{*} = \log (y_i)$:  
$$
...
inits <- list(xi = sample(1:10, size=consts$n, replace=TRUE), 
              ...
              s2Tilde = rinvgamma(consts$n, 1, 1), 
              alpha = 1) 
...
standlgrid <- seq(-2.5, 2.5, len = 200) # grid of values for standardized data in logarithmic scale
...
lines(standlgrid, apply(densitySamplesStandl, 2, mean), lwd = 2, col = 'black') # pointwise estimate of the density
...
densitySamplesl <- densitySamplesStandl / sd(lFaithful)

hist(faithful$waiting, freq = FALSE, xlim = c(40, 100), main = "", xlab = "Waiting times") 
...
The following code generates posterior samples from the random measure $G$. The 'cMCMC' object includes the model and posterior samples from the parameters. When running the getSamplesDPmeasure() function, a message displays the truncation level of $G$, say $truncG$. The resulting object is a matrix having $(truncG * (p+1))$ columns, where $p$ is the dimension of the vector of parameters with distribution $G$ (in this example $p=2$).
```{r, G-normaldensity}
outputG <- getSamplesDPmeasure(cmcmc) 
samplesG <- outputG$samples
```

The following code computes posterior samples of $P(\tilde{y} > 70)$ using the posterior samples from the random measure $G$. Note that these samples are computed based on the transformed model and a value larger than 70 corresponds to a value larger than -0.06281407 on the above defined grid, which is the 98th component.
...
hist(samples[ , 'alpha'], xlab = expression(alpha), ylab = "Frequency", main = "") # Posterior histogram
```
...
for(iter in seq_len(nrow(samples))) {
  densitySamples[iter, ] <- sapply(grid, function(x) sum(samplesG[iter, grep('weight', colnames(samplesG))] * dgamma(x, shape = samplesG[iter, grep('betaTilde', colnames(samplesG))], scale = samplesG[iter, grep('lambdaTilde', colnames(samplesG))]))) 
}
...
densitySamples <- matrix(0, ncol = length(grid), nrow = nrow(samples))
for(i in 1:nrow(samples)) {
  densitySamples[i, ] <- sapply(grid, function(x) sum(wSamples[i, ]*dgamma(x, 
