## Sampler for a standard deviation (typically) on
## a log scale that shifts the magnitude of a set of
## other nodes by the same value, also on a log scale.
## This helps mixing for a set of random effects and their
...
sampler_RW_log_shift <- nimbleFunction(
    ...
    setup = function(model, mvSaved, target, control) {
        ...
        scale         <- control$scale
        if(is.null(adaptive)) adaptive <- TRUE
        if(is.null(adaptInterval)) adaptInterval <- 100
